...
/// \brief 读取相机此刻的时间戳。
/// \param [in] hCamera 相机的句柄。
/// \param [out] pTimeStampL32 时间戳低32位。
/// \param [out] pTimeStampH32 时间戳高32位。
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
/// \~english
/// \brief Read the timestamp of the camera at this moment.
/// \param [in] hCamera Camera handle.
/// \param [out] pTimeStampL32 timestamp low 32bit。
/// \param [out] pTimeStampH32 timestamp high 32bit。
/// \return Returns CAMERA_STATUS_SUCCESS(0) successfully. Otherwise, it returns a non-zero error code. Please refer to the definition of the error code in CameraStatus.h.
MVSDK_API CameraSdkStatus CameraGetDevTimeStamp(
    CameraHandle    hCamera,
    UINT*       pTimeStampL32,
    UINT*       pTimeStampH32
);
...
/// \return Returns CAMERA_STATUS_SUCCESS(0) successfully. Otherwise, it returns a non-zero error code. Please refer to the definition of the error code in CameraStatus.h.
MVSDK_API CameraSdkStatus CameraDrawText(
	BYTE*           pRgbBuffer,
	tSdkFrameHead*  pFrInfo,
